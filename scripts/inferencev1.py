        ...
        with precision_scope("cuda"):
            with model.ema_scope():
                for filename in file_names:
                    ...
                    reference_path = filename.replace("_input.jpg", "_ref.jpg")
                    ref_p = Image.open(reference_path).convert("RGB").resize((224,224))
                    ref_tensor=get_tensor_clip()(ref_p)
                    ref_tensor = ref_tensor.unsqueeze(0)
                    mask_path = filename.replace("_input.jpg", "_mask.jpg")
                    mask=Image.open(mask_path).convert("L")
                    mask = mask.resize((opt.H, opt.W))
                    ...
                    if not opt.skip_save:
                        for i,x_sample in enumerate(x_checked_image_torch):
                            ...
                            mask_save.save(os.path.join(sample_path, os.path.basename(filename)[:-4]+'_'+str(opt.seed)+"_mask.png"))
                            GT_img=255.*rearrange(all_img[0], 'c h w -> h w c').numpy()
                            GT_img = Image.fromarray(GT_img.astype(np.uint8))
                            GT_img.save(os.path.join(sample_path, os.path.basename(filename)[:-4]+'_'+str(opt.seed)+"_GT.png"))
                            inpaint_img=255.*rearrange(all_img[1], 'c h w -> h w c').numpy()
                            inpaint_img = Image.fromarray(inpaint_img.astype(np.uint8))
                            inpaint_img.save(os.path.join(sample_path, os.path.basename(filename)[:-4]+'_'+str(opt.seed)+"_inpaint.png"))
